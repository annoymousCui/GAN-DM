...
class ContentFlatFolderDataset(data.Dataset):
    def __init__(self, root, transform):
        super(ContentFlatFolderDataset, self).__init__()
        self.root = root
    ...
    def name(self):
        return 'ContentFlatFolderDataset'

class StyleFlatFolderDataset(data.Dataset):
    def __init__(self, root, transform):
        super(StyleFlatFolderDataset, self).__init__()
        self.root = root
        self.paths = os.listdir(self.root)
        self.transform = transform
...
parser.add_argument('--sample_path', type=str, default='./samples/stage1', help='Derectory to save the intermediate samples')

# training options
parser.add_argument('--save_dir', default='./experiments/stage1',
...
style_dataset = StyleFlatFolderDataset(args.style_dir, style_tf)
content_dataset = ContentFlatFolderDataset(args.content_dir, content_tf)

content_sampler = RandomSampler(content_dataset)

content_iter = iter(data.DataLoader(
    content_dataset, batch_size=int(args.batch_size / 2),
    sampler=InfiniteSamplerWrapper(content_sampler),
...
for i in tqdm(range(args.start_iter, args.max_iter)):
    ...
    content_images, content_masks = next(content_iter)
    content_images = content_images.to(device)
    content_masks = content_masks.to(device)

    ######################################################
    content_masks_ = content_masks[1:]
    content_masks_ = torch.cat([content_masks_, content_masks[0:1]], 0)
    content_masks = torch.cat([content_masks, content_masks_], 0)
    ...
    img, loss_c, loss_s, l_identity1, l_identity2, loss_contrastive_c, loss_contrastive_s, xy_loss = network(
        content_images, style_images, content_masks, args.batch_size)
    ...
    loss = loss_c + loss_s + l_identity1 * 50 + l_identity2 * 1 + loss_contrastive_c + loss_contrastive_s + loss_gan_g + 20 * xy_loss
    ...
    if (i == 0) or ((i + 1) % 500 == 0):
        ...
        output_name = output_dir / 'output{:d}.png'.format(i + 1)
