...
class Net(nn.Module):
    ...
    def forward(self, content, style, masks, batch_size):
        ...
        hot_g_t = self.conv3(g_t)
        hot_g_t = torch.sigmoid(hot_g_t).cpu()
        hot_g_t_list = list(hot_g_t.view(-1, 256, 256))
        mask_list = list(masks.view(-1, 256, 256))

        coords_list = [set() for _ in range(18)]

        for i, tensor in enumerate(mask_list):
            nonzero_coords = tensor.nonzero()

            for coord in nonzero_coords:
                coords_list[i].add((coord[0].item(), coord[1].item()))

        coords_tuple_list = [tuple(coords) for coords in coords_list]

        total_distance = 0
        for j in range(18):
            per_distance = 0

            for i, coords in enumerate(coords_tuple_list[j]):
                row, col = coords
                tensor = hot_g_t_list[j]

                patch = tensor[max(0, row - 7):row + 8, max(0, col - 7):col + 8]
                min_value, min_coords = patch.reshape(-1).min(0)
                min_row, min_col = min_coords // patch.shape[1], min_coords % patch.shape[1]
                distance = math.sqrt(
                    (row - (min_row + max(0, row - 3))) ** 2 + (col - (min_col + max(0, col - 3))) ** 2)

                per_distance += distance
            if i == 0:
                continue
            per_distance /= i
            total_distance += per_distance
        total_distance /= 18

        g_t_feats = self.encode_with_intermediate(g_t)
        loss_c = self.calc_content_loss(g_t_feats[3], content_feats[3]) + self.calc_content_loss(
            g_t_feats[4], content_feats[4])
        ...
        return g_t, loss_c, loss_s, l_identity1, l_identity2, content_contrastive_loss, style_contrastive_loss, total_distance